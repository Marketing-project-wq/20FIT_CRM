...
describe("describePresence — the 'Filter terbaca' line must state exclusions (Track A binding)", () => {
  ...
  it("Track A opening segment reads as a plain sentence with the exclusions", () => {
    ...
    const c = crit({ ecoUnit: "event", exclude: { ecoUnit: "membership", srcArena: true, srcGym: false, srcHyrox: false, srcMy20fit: false, srcRecency: false, tagsAny: [] } });
  ...
  it("no-app segment (exclusion-only) reads clearly", () => {
    const c = crit({ exclude: { ecoUnit: null, srcArena: false, srcGym: false, srcHyrox: false, srcMy20fit: true, srcRecency: false, tagsAny: [] } });
  ...
  it("an exclusion is NEVER silently dropped — every active exclusion appears", () => {
    const c = crit({ ecoUnit: "event", exclude: { ecoUnit: "membership", srcArena: true, srcGym: true, srcHyrox: true, srcMy20fit: true, srcRecency: true, tagsAny: [] } });
