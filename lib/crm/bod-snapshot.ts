...
/**
 * PURE half of the board-summary data layer — NO I/O, no `server-only`, importable from a client
 * component. The split is not cosmetic: `components/dashboard/bod-content.tsx` renders inside the
 * client-side Dashboard, and importing the server-only module from there fails the production build
 * outright (it did, on the first attempt at this restructure). Fetching lives in `bod.ts`.
 *
 * Data layer for the board summary. EVERY figure comes from ONE daily snapshot —
