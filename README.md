...
> | — | `…20260825150000_revoke_anon_authenticated_crm_loose_grants` | *(not yet applied — GATED)* | **PENDING APPLY** (K-47 / T-35 / B10b). Revokes `anon`+`authenticated` from the 7 era-2B `crm_*` tables (audit_log, consent, profile_behavior, profile_demographic, profile_scores, suppression, user_role) → `{postgres, service_role}` only, matching the 6 newer tables. Data untouched; dependency-checked (0 policy / 0 view / 0 role inheritance; the 248-row batch was written via BYPASSRLS, which keeps its grant). Apply through the gated flow (show → confirm → apply → verify ACL → stamp ledger); guard `scanCrmTableGrantsToAnonAuth` blocks any future re-open. |
>
