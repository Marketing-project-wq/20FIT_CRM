...
> ## ✅ URUTAN DEPLOY migrasi 38 & 39 — SUDAH dijalankan dengan benar (3 Sep 2026)
>
...
> `crm_campaign_run.status IN ('partial','failed')` (K-55).
>
> **Kedua migrasi sudah diterapkan lebih dulu** (ledger `20260903125132` lalu `20260903125151`,
> satu per satu lewat `apply_migration`, masing-masing diverifikasi ulang dari `pg_constraint`), jadi
> produksi sudah menerima nilai barunya **sebelum** kodenya bisa tayang. Urutan ini bukan formalitas:
> kalau kodenya tayang lebih dulu, kirim yang ter-throttle gagal saat UPDATE dan barisnya tertinggal
> berstatus `queued`, dan `finalizeRunStatus` melempar saat menulis `partial`/`failed` — run selesai
> tapi statusnya tak pernah diperbarui. Arah sebaliknya (migrasi dulu) aman: CHECK yang lebih longgar
> tak mengganggu kode lama, yang memang tak pernah menulis nilai barunya.
...
> | 23 | `…20260821060000_unify_identity_sources_aplus` | `20260821154415` | `unify_identity_sources_aplus` (Migrasi 23 / jalur A+ — unifies identity sources into the mirror; merged via PR #14) |
...
> | 30 | `…20260825150000_revoke_anon_authenticated_crm_loose_grants` | `20260825164301` | `revoke_anon_authenticated_crm_loose_grants` (K-47 / T-35 / B10b). Revoked `anon`+`authenticated` from the 7 era-2B `crm_*` tables (audit_log, consent, profile_behavior, profile_demographic, profile_scores, suppression, user_role) → `{postgres, service_role}` only, matching the 6 newer tables. **Applied + verified 25 Aug 2026**: post-apply all 13 crm_* tables are `{postgres, service_role}`, RLS still ON on all 13, 0 tables grant anon/authenticated. Data untouched; dependency-checked before apply (0 policy / 0 anon-owned view / 0 role inheritance; the 248-row batch was written via BYPASSRLS, which keeps its grant). Local file stamp `20260825150000` differs from the applied ledger stamp `20260825164301` (apply time) — same divergence pattern as migration 15/28. Guard `scanCrmTableGrantsToAnonAuth` blocks any future re-open. |
> | 30a | `…20260826090000_seed_default_email_template` | **(TIDAK ADA STEMPEL)** | Seed dua baris template `default_newsletter` (id + en) ke `crm_message_template`. **Terbukti dijalankan**: kedua baris ada di produksi (`version 1`, kini `is_active=false` — dinonaktifkan belakangan, bukan tanda gagal apply). Tak ada stempel apa pun di `schema_migrations` — lihat T-47. |
> | 30b | `…20260826100000_crm_email_unsubscribe` | **(TIDAK ADA STEMPEL — DAN TIDAK PERNAH DITERAPKAN)** | `create table public.crm_email_unsubscribe` (log unsubscribe per (email, template), append-only, RLS on / grants service_role). **`to_regclass('public.crm_email_unsubscribe')` = NULL — tabelnya TIDAK ADA di produksi.** Satu-satunya berkas dari sapuan ini yang benar-benar tak pernah dijalankan. Tidak ada kode aplikasi yang merujuknya (dipindai 3 Sep 2026: nol rujukan di luar berkas migrasinya sendiri), jadi tak ada yang rusak hari ini. **Jangan diterapkan tanpa keputusan** — lihat T-47. |
> | 30c | `…20260826110000_crm_test_recipient` | **(TIDAK ADA STEMPEL)** | `crm_test_recipient` — alamat @20fit.id untuk uji kirim pra-luncur. **Terbukti dijalankan**: tabel ada, **1 baris**. |
> | 30d | `…20260827090000_crm_activity_layer` | **(TIDAK ADA STEMPEL)** | Lapisan aktivitas: `crm_activity_event` + `crm_customer_activity` + fungsi `crm_rebuild_activity_events` / `crm_refresh_customer_activity`. **Terbukti dijalankan**: kedua tabel ada (**1.525** dan **741** baris), kedua fungsi ada di katalog. Berkasnya menamai dirinya "MIGRASI 27" secara internal — itu penomoran sesi lain, **bukan** baris 27 ledger ini (`crm_segment`). |
> | 30e | `…20260827100000_ingest_activity_people` | **(TIDAK ADA STEMPEL)** | `crm_ingest_activity_people(...)` — ingest orang aktif ke `master_customer`. **Terbukti dijalankan**: fungsi ada di katalog (1 overload). Nama berkasnya tanpa awalan `crm_`, jadi rekonsiliasi per-nama harus menyertakannya secara eksplisit. |
> | 30f | `…20260827110000_crm_workflow` | **(TIDAK ADA STEMPEL)** | `crm_workflow` + `crm_workflow_enrollment`. **Terbukti dijalankan**: keduanya ada (**1** dan **36** baris) — dan migrasi 32 (`crm_campaign_run_owner_xor`, berstempel) punya FK ke `crm_workflow(id)`, jadi tabel ini sudah ada sebelum 31 Agu. |
> | 30g | `…20260828090000_crm_brand_asset` | **(TIDAK ADA STEMPEL)** | `crm_brand_asset` + bucket Storage publik. **Terbukti dijalankan**: tabel ada (**1** baris), bucket `brand-assets` ada dan `public = true`. |
> | 30h | `…20260828100000_crm_scheduled_send` | **(TIDAK ADA STEMPEL)** | `crm_scheduled_send` + eksekutor pg_cron. **Terbukti dijalankan**: tabel ada (**1** baris), dan job `crm-run-scheduled-sends` (jobid 18, `*/5 * * * *`, aktif) ada di `cron.job`. |
> | 31 | `…20260831051450_crm_workflow_trigger_source` | `20260831051450` | `crm_workflow_trigger_source` (workflow pemetaan, docs/PETA-WORKFLOW.md §9). Adds `crm_workflow.trigger_source text NOT NULL DEFAULT 'activity'` CHECK `('activity','pool')` so a `welcome` workflow can trigger from `master_customer.created_at` (new-pool welcome, full coverage) instead of only `crm_customer_activity.joined_at` (0.88% layer). `type` still holds the message meaning; the column separates where the time is read from. Backward-compatible: default `activity` keeps old behavior; **Applied + verified 31 Aug 2026** — column `text NOT NULL DEFAULT 'activity'`, check `('activity','pool')`, `crm_workflow` 0 rows (no backfill). **NB (diperbarui 3 Sep 2026): sapuan rekonsiliasi itu SUDAH dijalankan — hasilnya baris 30a–30h di atas. Kedelapan berkas 26–28 Agu TIDAK punya stempel `schema_migrations` sama sekali; tujuh di antaranya terbukti sudah diterapkan (objek + datanya hidup), satu (`crm_email_unsubscribe`) tak pernah diterapkan. Rincian + akibatnya untuk `supabase db push`: T-47.** |
> | 32 | `…20260831055527_crm_campaign_run_owner_xor` | `20260831055527` | `crm_campaign_run_owner_xor` (T-38 fix). A run now belongs to EITHER a segment OR a workflow: `segment_id` made **nullable**, new nullable `workflow_id uuid` FK→`crm_workflow(id)` ON DELETE RESTRICT, XOR constraint `check (num_nonnulls(segment_id, workflow_id) = 1)`, index `crm_campaign_run_workflow_idx (workflow_id, template_key)`. Before this, `runWorkflowAction` passed the workflow id as `segment_id` → FK violation → `run_create_failed` masked as "cek konfigurasi kirim"; every workflow send died there. **Applied + verified 31 Aug 2026**: verified BEFORE apply the table had **12 rows, all segment-owned** (my first note said 0 — corrected by owner); XOR holds because every legacy row is a campaign run (`num_nonnulls`=1), not because the table was empty. Verified AFTER apply: both cols nullable, XOR def `CHECK ((num_nonnulls(segment_id, workflow_id) = 1))`, FK `→ crm_workflow(id) ON DELETE RESTRICT`, index present, **12/12 rows satisfy the XOR**. |
> | 33 | `…20260831081344_crm_send_config` | `20260831081344` | `crm_send_config` (owner request — configurable send limits). Singleton row (`id boolean pk default true`, `check (id = true)`) holding `daily_limit` (system daily ceiling) + `workflow_daily_cap` (sub-cap for automated workflow sends), each `> 0`, with a DB-enforced `check (workflow_daily_cap <= daily_limit)`. Replaces the hard-coded `DEFAULT_SEND_CONFIG.dailyLimit=1000` / workflow cap `300`. Editable by Super Admin (audited). **Applied + verified 31 Aug 2026**: row `{1000, 300}`, 4 check constraints, RLS on, 0 policy, grants `{postgres, service_role}` only. |
> | 34 | `…20260901031500_crm_backfill_run_labels` | `20260901074234` | Backfill legacy NULL campaign names (data-only, idempotent). Fills `crm_campaign_run.label` (8 rows) + `crm_scheduled_send.run_label` (1 row) `WHERE IS NULL` with the app's human default `"{segment} · {DD Mon YYYY} {HH:MM}"` (Asia/Jakarta WIB, Indonesian month abbrev, **never** an ISO timestamp); rows with no resolvable segment get a `"Kampanye · …"` fallback. The WIB `HH:MM` of `created_at` is appended to **every** backfilled row so 6 same-segment/same-day legacy runs stay distinct before the column goes NOT NULL. Each UPDATE guarded by `IS NULL` (re-run touches nothing already named). **The 9 produced values were reviewed & approved by the owner before applying. Applied + verified 1 Sep 2026**: `run_null=0`, `sched_null=0`, all 9 labels distinct (`dup_count=1` each), zero diff from the approved table. |
> | 35 | `…20260901032000_crm_run_label_not_null` | `20260901081307` | `SET NOT NULL` on `crm_campaign_run.label` and `crm_scheduled_send.run_label` — enforces "every campaign has a name" at the DB now that new runs require a validated name and the legacy NULLs were backfilled in migration 34. Ordered strictly **after** 34 (nulls filled first). Safe against the scheduled-send cron: it writes a name via `cronRunLabel` (`s.runLabel?.trim() || defaultCampaignLabel(...)`, never empty, never ISO) — the whitespace seam a bare `??` would miss was closed in the same PR. Pre-apply check: `pending_total=0` (no due row the live cron could turn into a NULL insert in the pre-deploy window). **Applied + verified 1 Sep 2026**: both columns `is_nullable = NO`; cron cycle after the constraint confirmed `succeeded`. |
> | 36 | `…20260901083000_crm_cleanup_uji_iso_labels` | `20260901083824` | Data-only cleanup of 3 legacy `crm_campaign_run` names carrying a raw ISO timestamp (`"UJI kirim internal 2026-08-25T…Z"`, ids `7eb5ddb7`/`ade3fb47`/`232a5d50`). These are NOT the cron artifact — they came from the **old manual composer's auto-generate** (segment name + ISO), created 25 Aug 2026 before name became mandatory; that write path is dead. Rewritten to the same human default as migration 34 (`"{segment} · DD Mon YYYY HH:MM"` WIB). **Scoped + idempotent**: touches only those 3 ids, guarded `label ~ '\d{4}-\d{2}-\d{2}T'` (re-run = no-op). Owner reviewed the 3 values before applying. **Applied + verified 1 Sep 2026**: all 3 rewritten, and the **single** cron-path ISO row `2d030cb4` was rewritten earlier this session by direct guarded SQL (not ledgered as a migration; production-only row) → post-cleanup **0 ISO-shaped labels remain** in either table. |
> | 37 | `…20260902050000_crm_ingest_csv_people` | `20260907043325` | `crm_ingest_csv_people(jsonb, uuid, text, uuid, jsonb)` — jalur tulis impor audiens CSV (Fase 1). `SECURITY DEFINER`, `search_path=public`, EXECUTE **hanya** `service_role`. Menyisipkan orang benar-benar baru (kolom aman saja) dengan `source='csv_import'` + `tags=['csv_import','batch:<uuid>']`, dedup **email-primer** (K-57 — cocok telepon saja = orang lain, tetap disisipkan dengan telepon dikosongkan), plus satu baris `crm_consent` bukti per orang (`basis='explicit_opt_in'`). Orang yang **sudah ada** ditandai `tagged:<uuid>` pada kolom `tags` saja (K-58) — penanda sengaja beda bentuk dari `batch:` supaya rollback per-batch tak pernah menghapus pelanggan milik batch lain (T-52). Tag divalidasi regex yang identik dengan `OPERATOR_TAG_REGEX_SOURCE` di `lib/crm/tags.ts`; tag di luar kosakata **menggagalkan panggilan** (SQLSTATE 22023), tak pernah dibuang diam-diam. Membuat fungsi saja — **nol impor dijalankan**. **Percobaan apply PERTAMA (7 Sep 2026) DITOLAK**: `42601 syntax error at or near "tag_targets"` — satu koma hilang antara CTE `ins` dan `tag_targets`. Nol dampak: galat waktu-urai, sebelum DDL berlaku, dan ledger tidak distempel untuk migrasi gagal (keenam angka diukur ulang, identik). Lihat **T-54**, dan pagar keenam (`lib/crm/migration-parse-guard.test.ts`) yang lahir darinya. **Diterapkan + diverifikasi 7 Sep 2026** setelah koma diperbaiki (diff disetujui pemilik): `pg_proc` → **1 overload, nol lainnya**, tanda tangan `(jsonb,uuid,text,uuid,jsonb)`, `prosecdef=true`, `proconfig={search_path=public}`, `proacl={postgres=X/postgres,service_role=X/postgres}` → `anon`=false, `authenticated`=false, `public`=false, `service_role`=true. **Panggilan sungguhan di produksi, di dalam transaksi yang dipaksa rollback**: kembalian `{inserted 4, tagged_existing 1, shared_phone_in_batch 2}`; 4 baris consent `explicit_opt_in`; telepon yang bentrok dengan master dikosongkan dan **tidak** dihitung sebagai telepon-ganda-dalam-berkas; pada orang yang sudah ada perbandingan **seluruh 24 kolom** menunjukkan **hanya `tags`** yang berubah (`updated_at` tidak), consent-nya tetap 5 → 5; tiga penolakan tag (`vip`, injeksi `batch:<uuid>`, huruf besar) semuanya **SQLSTATE 22023**. Pasca-rollback: `master_customer` **82.830**, `crm_consent` **408.119**, nol baris uji tersisa, `source='csv_import'` = **0**. Stempel ledger `20260907043325` ≠ nama berkas `20260902050000` (waktu apply) — pola divergensi yang sama dengan migrasi 15/28/30/38/39. **Impor sungguhan masih menunggu gerbang pemilik.** SQL rollback + alasannya di `docs/RENCANA-impor-audiens.md`. |
> | 38 | `…20260903090000_crm_message_log_add_provider_throttled` | `20260903125132` | Migrasi A / T-41 / K-54 — menambah `provider_throttled` ke `crm_message_log_failure_cause_check`. 429/402/503 = provider men-throttle **kami**, bukan menolak **penerima**; `provider_rejected` tetap untuk 4xx level-penerima. Drop + recreate constraint dengan nama sama dalam satu transaksi. **Diterapkan + diverifikasi 3 Sep 2026**: pra-apply `crm_message_log` = 18.247 baris, run `draft`/`sending` = 0, `crm_scheduled_send` pending = 0, nilai `failure_cause` terpakai hanya `(null)`/`hard_bounce`/`unknown` (semuanya legal di CHECK lama & baru). Pasca-apply dibaca ulang dari `pg_constraint`: array kini `{invalid_address, hard_bounce, provider_rejected, provider_throttled, daily_limit, unknown}`, `convalidated = true`, **18.247 baris tetap, nol ditulis ulang**, `provider_throttled` = 0 baris. `daily_limit` dibawa apa adanya — tak pernah ditulis kode mana pun (dikonfirmasi ulang 3 Sep 2026). Stempel ledger `20260903125132` **berbeda** dari nama berkas `20260903090000` (waktu apply) — pola divergensi yang sama dengan migrasi 15/28/30. SQL + rollback ada di berkasnya. |
> | 39 | `…20260903091000_crm_campaign_run_add_partial_failed` | `20260903125151` | Migrasi B / T-42 / K-55 — menambah `partial` + `failed` ke `crm_campaign_run_status_check`. **Diterapkan + diverifikasi 3 Sep 2026**: pasca-apply dibaca ulang dari `pg_constraint` → `CHECK ((status = ANY (ARRAY['draft','sending','sent','stopped','partial','failed'])))`, `convalidated = true`. **Tanpa backfill**: 14 baris run tak satu pun ditulis ulang; run `5f5f3a57` tetap `status='sent'`, dan query setara `listResumableRuns` untuk pasangan (segmen `cd20a01f`, template `email_1788335441757`) mengembalikan **nol baris** pasca-migrasi — tetap tidak resumable. Keduanya juga tidak resumable secara desain: `RESUMABLE_RUN_STATUSES = {draft, sending}`, dijaga di `listResumableRuns`, diperiksa ulang di `getRunForPair`, dikunci test pemindai sumber. Stempel ledger `20260903125151` ≠ nama berkas `20260903091000` (waktu apply). SQL + rollback ada di berkasnya. |
> | 40 | `…20260907060000_crm_mirror_bod_daily_stats` | `20260907061349` | `crm_mirror_bod_daily_stats` (K-61 / K-63 / T-59). Menambahkan **empat kunci** ke `crm_mirror_meta.dashboard_stats` — `reach`, `loads`, `delivery`, `not_in_crm` — supaya seluruh layar BOD menjadi satu potret harian dengan satu cap waktu. **Aditif**: keenam kunci lama (`ecosystem`, `engagement`, `fitco`, `rfm`, `sources`, `candidates`) direproduksi byte-per-byte dari `pg_get_functiondef`; nol `alter`/`drop`/`delete`. **Diterapkan + diverifikasi 7 Sep 2026**: pra-apply blob enam kunci (`refreshed_at` 2026-09-06 20:00 UTC), nol stempel, 1 overload, acl `{postgres, service_role}`. Pasca-apply `pg_proc` → 1 overload, `prosecdef=true`, `proconfig={search_path=public}`, anon/authenticated/public false, service_role true, **acl tak berubah**. Fungsinya lalu **dijalankan sungguhan** (bukan sekadar dibuat — T-48): blob kini **10 kunci**, dan keempat kunci baru dibandingkan satu per satu dengan hitungan aplikasi — `reach` 82.213/81.679/82.830/126, `delivery` 121/5/18.119/1/36, semuanya **cocok**; `loads` tiga muatan (81.178/1.075/577); `not_in_crm.distinct_people` 1.376; `engagement` dan `candidates` **tidak berubah** (candidates.total 2.799 — populasi berbeda dari not_in_crm, sengaja tidak dipakai untuk kartu 5). Performa diukur sebelum apply: reach 1.601 ms → **53 ms** setelah anti-join berkorelasi diganti hash left join; `not_in_crm` 166 ms. **Membaca sepuluh tabel milik divisi lain** (arena/gym/klinik/my20fit/hyrox) — baca-saja, agregat saja; konsekuensinya dicatat di **T-61**. Rollback = berkas siap jalan `docs/riwayat/ROLLBACK-crm_refresh_customer_mirror-20260907.sql` (definisi enam-kunci verbatim); kalau dijalankan, halaman BOD **wajib** ikut dikembalikan di deploy yang sama — `parseBodSnapshot` melempar galat pada blob enam-kunci, sengaja, supaya nol tak pernah tampil sebagai hasil pengukuran. Stempel ledger `20260907061349` ≠ nama berkas `20260907060000` (waktu apply) — pola divergensi yang sama dengan 15/28/30/37/38/39. |
>
> | 41 | `…20260909050000_crm_message_template_add_sender_name` | `20260909060215` | `crm_message_template.sender_name` (T-74 / K-65). Kolom nama pengirim per-template. Nama pengirim dulu di-hardcode `"20FIT CRM"` di `lib/email/mailtrap.ts` dan diwarisi diam-diam oleh jalur kampanye yang dibangun di atas fungsi kirim email reset; kolom ini memberi kampanye nama pengirimnya sendiri, jalur reset tetap default. **ADITIF & nullable**: baris lama tetap NULL → jalur kirim jatuh ke default `"20FIT CRM"`. Batas panjang (`MAX_SENDER_NAME=64`, dinalar bukan terukur — kolom baru, 0 baris) + pembersihan baris-baru/spasi ditegakkan DI RUTE (`POST /api/templates`, jalur tulis satu-satunya), pola full_name/city crm_update_master_fields. **Diterapkan + diverifikasi 9 Sep 2026**: `information_schema` → `sender_name text`, is_nullable `YES`, default null; **23 baris, 0 ber-sender_name** (semua NULL). Rantai penuh dikunci `lib/crm/sender-name-chain.test.ts` (terbukti merah bila lapis mana pun membuang nilainya). Stempel ledger `20260909060215` ≠ nama berkas `20260909050000` (waktu apply) — pola divergensi 15/28/30/37/38/39/40. |
>
...
> hole).
>
> **Do NOT run `supabase db push` against this project. Re-measured 3 Sep 2026 — masih TIDAK
> aman, dan alasannya kini terukur, bukan diperkirakan.** Sapuan rekonsiliasi penuh (repo ↔
> `schema_migrations`, difilter per NAMA) menemukan:
>
> - **47 berkas migrasi CRM di repo**, dicocokkan per nama ke `schema_migrations`:
>   **17 stempelnya identik** dengan nama berkas (CLI akan melewatinya dengan benar),
>   **21 stempelnya divergen** (waktu apply ≠ nama berkas), **9 tanpa stempel sama sekali**.
>   Artinya **30 dari 47** akan dianggap belum diterapkan oleh CLI dan dicoba dijalankan ulang —
>   termasuk pembuatan tujuh tabel `crm_*` era-2B dan setiap `create or replace` fungsinya.
> - **8 berkas (26–28 Agu, baris 30a–30h) TIDAK punya stempel sama sekali**, tujuh di antaranya
>   sudah hidup di produksi berikut datanya. `db push` akan mencoba membuat ulang `crm_workflow`,
>   `crm_activity_event`, `crm_scheduled_send`, dst.
> - **1 berkas (`20260902050000_crm_ingest_csv_people`) memang BERGATE** dan belum boleh jalan —
>   `db push` akan menerapkannya tanpa persetujuan siapa pun. Ini bahaya yang paling langsung:
>   bukan galat "already exists", melainkan jalur tulis impor yang tayang tanpa gerbang.
> - **0 stempel yatim** (tak ada versi CRM di ledger yang tak punya berkas repo) — jadi
>   ketidakcocokannya satu arah saja: berkas tanpa stempel.
>
> Sebagian besar berkas memakai `create table if not exists` / `create or replace`, jadi banyak yang
> akan lolos diam-diam alih-alih gagal keras — itu **lebih buruk**, bukan lebih baik: data seed
> disisipkan ulang, grant dan policy ditulis ulang, dan tak ada yang menandai bahwa itu terjadi.
> Jalankan setiap migrasi satu per satu lewat jalur bertinjau (`apply_migration`), bukan `db push`.
> Syarat agar `db push` aman suatu hari nanti ada di **T-47**.
>
